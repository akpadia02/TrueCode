...
const AddProduct = () => {    
    ...
    Geocode.setApiKey('67de6baac198a404378221mhqec3eba')
    useEffect(() => {
        Geocode.fromLatLng(manuLatitude, manuLongtitude).then(
            (response) => {
              const address = response.results[0].formatted_address;
    ...
    const generateQRCode = async (serialNumber) => {
        ...
        // const qrCode = await productContract.getProduct(serialNumber);
        if (serialNumber && name && brand && description ) {
        ...
        const apiUrl = `https://api.qrserver.com/v1/create-qr-code/?data=${encodedData}&size=200x200`;
        console.log("QR Code: ", qrData);
        setQrUrl(apiUrl);
    }
    }
    ...
    const downloadQR = async () => {
        if (!qrUrl) return alert("Generate the QR code first!");
        ...
        try {
            ...
            link.href = url;
            link.download = `QR_${serialNumber || "code"}.png`;
    ...
    return (
        <Box sx={{
            background: "linear-gradient(to right, #0a192f, #1b2838)",
            minHeight: "80vh",
            backgroundRepeat: "no-repeat",
            position: 'absolute',
            left: 0,
            ...
            <Paper elevation={3} sx={{ width: "400px", margin: "auto", marginTop: "10%", marginBottom: "10%", padding: "3%", backgroundColor: "#e3eefc" }}>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    {qrUrl !== "" ? <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', marginTop: '3%' }}>
                        <Button
                            variant="outlined"
                            component="label"
                            fullWidth
...
//             overflowY: "scroll"
//         }}>
//             <Paper elevation={3} sx={{ width: "400px", margin: "auto", marginTop: "10%", marginBottom: "10%", padding: "3%", backgroundColor: "#ffffff" }}>
//                 <Typography
//                     variant="h2"
//                     sx={{
//                         textAlign: "center", marginBottom: "3%",
